...
First do the plot to show how many major and minor spikes there are. Just using $\texttt{add_markers}$, $\texttt{add_lines}$, and $\texttt{layout}$ to make a similar plot.

```{r}
library(plotly)
library(dplyr)

#remove the S4 mean from earlier in Problem 1
removeGeneric("mean")
...
plot_ly() %>%
  add_markers(data = covid, x = ~date, y = ~cases, name = "Cases") %>%
  add_lines(data = case_mean, x = ~date, y = ~cases_avg, line = list(color = "red", width = 3),
    name = "Mean Cases") %>%
  layout(title = "US Covid Cases", xaxis = list(
      title = "Date",
      tickformat = "%b %Y",
      dtick = "M6"),
...
Now for the wrapped plots that are by state, we can use $\texttt{subplot}$.
...
#function to build the list of plots to feed to subplot
plots <- lapply(unique(case_states$state), function(st) {
  
  each_state <- case_states %>% filter(state == st)
  
  plot_ly(each_state, x = ~date, y = ~cases, type = "scatter", mode = "markers", name = st) %>%
    layout(title = st, xaxis = list(title = list(text = "Date", standoff = 10), tickformat = "%Y"), 
           yaxis = list(title = "Cases")
    )
})

#make similar layout to facet with subplot
subplot(plots, nrows = 2, shareX = FALSE, shareY = FALSE, titleX = TRUE, titleY = TRUE) %>%
  layout(title = "Covid Cases", margin = list(t = 80, b = 40, l = 60, r = 40))
```

### c)

Now plots to see the first five states that experienced major spikes.

```{r}
state_list <- unique(covid$state)

#smaller groups since the plot gets too crowded
state_groups <- split(state_list, ceiling(seq_along(state_list)/6))


plots2 <- lapply(state_groups, function(group) {
  
  plots <- lapply(group, function(st) {
  each_state <- covid %>% filter(state == st)
...
#make similar layout to facet with subplot
subplot(plots, nrows = 2, shareX = FALSE, shareY = FALSE, titleX = TRUE, titleY = TRUE) %>%
  layout(title = "Covid Cases (Top 3 and Bottom 3 States by Rate)", margin = list(t = 80, b = 40, l = 60, r = 40))
